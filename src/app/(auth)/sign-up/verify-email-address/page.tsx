  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        // After successful verification, also update the user status in Supabase
        try {
          const response = await fetch("/api/update-user-verification", {
            method: "POST",
            headers: {
          ...
          if (!response.ok) {
            console.warn(
              "Failed to update user verification status in database",
            );
          }
        } catch (dbError) {
          console.warn("Error updating verification status:", dbError);
