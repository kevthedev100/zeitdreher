-- Final fix for time_entries and users relationship
...
-- Update the realtime publication to include time_entries and users ONLY if they're not already members
DO $$ 
BEGIN
  -- Check if supabase_realtime publication exists
  IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
    -- Check if time_entries is already a member of the publication
    IF NOT EXISTS (
    ...
    -- Check if users is already a member of the publication
    IF NOT EXISTS (
      SELECT 1 FROM pg_publication_tables 
      WHERE pubname = 'supabase_realtime' 
      AND schemaname = 'public' 
