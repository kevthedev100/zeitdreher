-- Fix the UUID vs text comparison issue in auth functions

-- Create or replace the auth.users trigger function to ensure proper type handling
...
AS $$
BEGIN
  -- Explicitly cast the id to UUID when comparing
  UPDATE auth.users
  SET last_sign_in_at = now()
  WHERE id::uuid = NEW.user_id::uuid;
  RETURN NEW;
END;
$$;

-- Ensure the auth.users table has the correct reference to public.users
ALTER TABLE IF EXISTS auth.users
  DROP CONSTRAINT IF EXISTS users_id_fkey;

-- Ensure the auth.users.id is properly typed as UUID
ALTER TABLE IF EXISTS auth.users
  ALTER COLUMN id TYPE uuid USING id::uuid;

-- Ensure the public.users.user_id is properly typed as UUID
